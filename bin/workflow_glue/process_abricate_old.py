...
################################################################################################################
# Last modified: 2025-02-14, Shania Wu
################################################################################################################
...
def parse_typing_file(typing_file):
    ...
    ## Only type as A or B if genome coverage > 50% for each genome
    if len(abricate_A_grouped) > 0:
        A_total_cov = abricate_A_grouped['%COVERAGE'].iloc[0]
        if A_total_cov > 50:
            print(f"Genome coverage for RSV_TYPE_A combined hits > 50% ({A_total_cov}%), type_as_A = True")
            type_as_A = True
        else:
            type_as_A = False
            print(f"Genome coverage for RSV_TYPE_A combined hits <= 50% ({A_total_cov}%), type_as_A = False")
    else:
        type_as_A = False
    ...
    if len(abricate_B_grouped) > 0:
        ...
        if B_total_cov > 50:
            print(f"Genome coverage for RSV_TYPE_B combined hits > 50% ({B_total_cov}%), type_as_B = True")
            type_as_B = True
        else:
            type_as_B = False
            print(f"Genome coverage for RSV_TYPE_B combined hits <= 50% ({B_total_cov}%), type_as_B = False")
    else:
        type_as_B = False
        print("No type B hits.")
   
   
    ## If A and B both dont have > 50% coverage, but one or both have <= 50% coverage
    if not type_as_A and not type_as_B:
        
        # there are hits <= 50% coverage for both A and B => mixed
        if len(abricate_A_grouped) > 0 and len(abricate_B_grouped) > 0: 
            print(f"Both RSV_TYPE_A ({A_total_cov}) and RSV_TYPE_B ({B_total_cov}) have hits <= 50% genome coverage, assigning as 'mixedAB'.")
            type_as_A = True
            type_as_B = True
    
        # only A has hits <= 50% coverage; B has no hits
        elif len(abricate_A_grouped) > 0 and len(abricate_B_grouped) == 0:
            print("Only RSV_TYPE_A has <= 50% genome coverage, typing as 'RSV_TYPE_A'.")
            type_as_A = True
            type_as_B = False
        
        # only B has hits <= 50% coverage; A has no hits
        elif len(abricate_A_grouped) == 0 and len(abricate_B_grouped) > 0:
            print("Only RSV_TYPE_B has <= 50% genome coverage, typing as 'RSV_TYPE_B'.")
            type_as_A = False
            type_as_B = True
        
        # no hits for A and B at any coverage (should have been caught by previous if abricate.empty check)
        else:
